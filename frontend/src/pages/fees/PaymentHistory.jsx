...
import { ArrowLeft, ArrowRight, Banknote, CalendarDays, CheckCircle2, ChevronLeft, ChevronRight, CircleDollarSign, Download, FileText, FilterX, GraduationCap, Pencil, Plus, ReceiptText, ReceiptText as ReceiptIndianRupee, RefreshCw, RotateCcw, Save, Search, Smartphone, WalletCards, X } from "lucide-react";
...
const PaymentHistory = () => {
  ...
  const reversePayment = async (event) => {
    event.preventDefault();
    if (!reversal?.id || reversal.reason.trim().length < 3) {
      toast.error("Reversal reason kam se kam 3 characters ka hona chahiye");
      return;
    }
    try {
      setReversalSaving(true);
      await feePaymentApi.reverse(reversal.id, reversal.reason.trim());
      setReversal(null);
      await fetchPayments();
      toast.success("Payment, membership balance aur income entry successfully reversed");
    } catch (error) {
      toast.error(error.response?.data?.message || "Payment reverse nahi hui");
    } finally {
    ...
    <div className={`page ${styles.page}`}>
      ...
      <section className={styles.filterCard}>
        ...
        <div className={styles.filters}>
          ...
          <label><span>Status</span><select value={filters.status} onChange={(event) => setFilters((prev) => ({ ...prev, status: event.target.value }))}><option value="">All Statuses</option><option value="paid">Paid</option><option value="partial">Partial</option><option value="due">Due</option><option value="cancelled">Cancelled</option></select></label>
        </div>
      </section>
      ...
      {reversal ? <div className={styles.modalBackdrop} onMouseDown={(event) => event.target === event.currentTarget && !reversalSaving && setReversal(null)}><form className={styles.dateModal} onSubmit={reversePayment}><header><div><small>Financial correction</small><h2>Reverse Payment</h2><p>{reversal.student} · {reversal.receipt}</p></div><button type="button" onClick={() => setReversal(null)} disabled={reversalSaving} aria-label="Close"><X size={18}/></button></header><label><span>Reversal Reason *</span><textarea rows="4" minLength="3" maxLength="300" value={reversal.reason} onChange={(event) => setReversal((current) => ({ ...current, reason: event.target.value }))} placeholder="Example: Incorrect payment entry" required /></label><footer><button type="button" onClick={() => setReversal(null)} disabled={reversalSaving}>Keep Payment</button><button type="submit" disabled={reversalSaving}><RotateCcw size={16}/>{reversalSaving ? "Reversing…" : "Reverse Payment"}</button></footer></form></div> : null}
    </div>
  );
};
