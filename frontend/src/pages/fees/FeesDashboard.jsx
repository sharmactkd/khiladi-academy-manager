...
import { branchFor, formatMoney, scopeCurrencySource } from "../../utils/currency.js";
...
const FeesDashboard = () => {
  ...
  const dashboardCurrencySource = scopeCurrencySource(branches) || { currencyCode: "MIX", currencySymbol: "MIX " };
  const currency = (value) => formatMoney(value, dashboardCurrencySource);
  const recentPaymentCurrency = (payment) => {
    const matchedBranch = payment?.branchId
      ? branchFor(branches, payment.branchId)
      : null;

    if (matchedBranch?.currencyCode || matchedBranch?.currencySymbol) {
      return matchedBranch;
    }

    if (payment?.branch?.currencyCode || payment?.branch?.currencySymbol) {
      return payment.branch;
    }

    // Legacy payments may contain the old INR snapshot. When no row branch is
    // available, the academy's currently selected main-branch currency wins.
    return mainBranch || payment;
  };
    ...
    <div className={`page ${styles.page}`}>
      ...
      {loading ? <div className={styles.loading}><RefreshCw size={25} /><strong>Loading financial overview...</strong></div> : <>
        ...
        <section className={styles.activityGrid}>
          <article className={styles.tableCard}><header><div><h2>Recent Payments</h2><p>Latest recorded fee transactions</p></div><Link to="/fees/payments">View all<ArrowRight size={13}/></Link></header>{!data.recentPayments?.length ? <div className={styles.empty}>No payments found for this academy.</div> : <div className={styles.tableWrap}><table><thead><tr><th>Student</th><th>Receipt</th><th>Payment Mode</th><th>Amount</th><th>Status</th><th>Date</th></tr></thead><tbody>{data.recentPayments.slice(0, 6).map((payment) => { const paymentCurrency = recentPaymentCurrency(payment); return <tr key={payment._id} onDoubleClick={() => payment._id && navigate(`/fees/receipt/${payment._id}`)}><td><span className={styles.avatar}>{String(payment.studentName || "S").charAt(0)}</span><div><strong>{payment.studentName || "—"}</strong><small>{payment.admissionNumber || "No admission number"}</small></div></td><td>{payment.receiptNumber || "—"}</td><td><div className={styles.paymentModeCell}><strong>{formatPaymentMode(payment.paymentMode)}</strong>{payment.paymentMode === "cash_online" ? <small>Cash {formatMoney(payment.cashAmount, paymentCurrency)} + Online {formatMoney(payment.onlineAmount, paymentCurrency)}</small> : null}</div></td><td><strong>{formatMoney(payment.amountPaid, paymentCurrency)}</strong></td><td><span className={`${styles.status} ${styles[`status${String(payment.status || "due")[0].toUpperCase()}${String(payment.status || "due").slice(1)}`]}`}>{payment.status || "due"}</span></td><td>{formatDate(payment.paymentDate)}</td></tr>; })}</tbody></table></div>}</article>
