    ...
    <div className={`page ${styles.page}`}>
      ...
      {loading ? <div className={styles.loading}><RefreshCw size={25} /><strong>Loading financial overview...</strong></div> : <>
        ...
        <section className={styles.analyticsGrid}>
          <article className={styles.chartCard}><header><div><h2>Collection Overview</h2><p>Last six fee periods ending {selectedMonth} {filters.year}</p></div><Link to="/analytics/fees">Detailed Analytics<ArrowRight size={13} /></Link></header><div className={styles.chart}><ResponsiveContainer width="100%" height="100%"><AreaChart data={data.monthlyTrend || []} margin={{ top: 16, right: 12, left: 2, bottom: 0 }}><defs><linearGradient id="feesArea" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#e50914" stopOpacity={0.2}/><stop offset="100%" stopColor="#e50914" stopOpacity={0}/></linearGradient></defs><CartesianGrid stroke="#e8edf3" strokeDasharray="3 4" vertical={false}/><XAxis dataKey="label" axisLine={false} tickLine={false} tick={{ fill: "#718096", fontSize: 10 }}/><YAxis axisLine={false} tickLine={false} tick={{ fill: "#718096", fontSize: 9 }} tickFormatter={(value) => value >= 100000 ? `${(value / 100000).toFixed(1)}L` : value >= 1000 ? `${Math.round(value / 1000)}k` : value}/><Tooltip formatter={(value) => [currency(value), "Collection"]} contentStyle={{ borderRadius: 9, border: "1px solid #dce4ed", fontSize: 10 }}/><Area type="monotone" dataKey="amount" stroke="#e50914" strokeWidth={2.5} fill="url(#feesArea)" dot={{ r: 3.5, fill: "#e50914", stroke: "#fff", strokeWidth: 2 }} activeDot={{ r: 5 }}/></AreaChart></ResponsiveContainer></div></article>
          <article className={styles.mixCard}><header><div><h2>Payment Mix</h2><p>{selectedMonth} collections by mode</p></div></header><div className={styles.mixBody}><div className={styles.donut}><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={paymentMix} dataKey="value" nameKey="name" innerRadius={48} outerRadius={70} paddingAngle={2} stroke="none">{paymentMix.map((item) => <Cell key={item.key} fill={item.color}/>)}</Pie><Tooltip formatter={(value) => currency(value)} contentStyle={{ borderRadius: 9, border: "1px solid #dce4ed", fontSize: 10 }}/></PieChart></ResponsiveContainer><div><strong>{currency(totalMix)}</strong><span>Total</span></div></div><ul>{paymentMix.map((item) => <li key={item.key}><i style={{ background: item.color }}/><span><b>{item.name}</b><small>{currency(item.value)} · {totalMix ? Math.round((item.value / totalMix) * 100) : 0}%</small></span></li>)}</ul></div></article>
        </section>

        <section className={styles.activityGrid}>
          <article className={styles.tableCard}><header><div><h2>Recent Payments</h2><p>Latest recorded fee transactions</p></div><Link to="/fees/payments">View all<ArrowRight size={13}/></Link></header>{!data.recentPayments?.length ? <div className={styles.empty}>No payments found for this academy.</div> : <div className={styles.tableWrap}><table><thead><tr><th>Student</th><th>Receipt</th><th>Payment Mode</th><th>Amount</th><th>Status</th><th>Date</th></tr></thead><tbody>{data.recentPayments.slice(0, 6).map((payment) => <tr key={payment._id} onDoubleClick={() => payment._id && navigate(`/fees/receipt/${payment._id}`)}><td><span className={styles.avatar}>{String(payment.studentName || "S").charAt(0)}</span><div><strong>{payment.studentName || "—"}</strong><small>{payment.admissionNumber || "No admission number"}</small></div></td><td>{payment.receiptNumber || "—"}</td><td><strong>{formatPaymentMode(payment.paymentMode)}</strong>{payment.paymentMode === "cash_online" ? <small>Cash {currency(payment.cashAmount)} + Online {currency(payment.onlineAmount)}</small> : null}</td><td><strong>{currency(payment.amountPaid)}</strong></td><td><span className={`${styles.status} ${styles[`status${String(payment.status || "due")[0].toUpperCase()}${String(payment.status || "due").slice(1)}`]}`}>{payment.status || "due"}</span></td><td>{formatDate(payment.paymentDate)}</td></tr>)}</tbody></table></div>}</article>
