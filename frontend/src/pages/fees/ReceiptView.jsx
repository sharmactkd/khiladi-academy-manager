  ...
  return (
    <div className={`page ${styles.page}`}>
      ...
      <main ref={receiptRef} className={styles.receipt}>
        ...
        <section className={styles.feeTable} aria-label="Fee payment summary"><div className={styles.tableHeader}><span>Description</span><span>Fee</span>{Number(payment.discount || 0) > 0 ? <span>Discount</span> : null}<span>Paid</span><span>Balance</span></div><div className={styles.tableRow}><strong>Monthly Training Fee — {feePeriod}</strong><span>{currency(payment.amount)}</span>{Number(payment.discount || 0) > 0 ? <span>{currency(payment.discount)}</span> : null}<span>{currency(payment.amountPaid)}</span><span>{currency(payment.pendingAmount)}</span></div></section>
        <section className={styles.paymentOverview}>
          ...
          <dl className={styles.totals}><div><dt>Subtotal</dt><dd>{currency(payment.amount)}</dd></div>{Number(payment.discount || 0) > 0 ? <div><dt>Discount</dt><dd className={styles.discount}>−{currency(payment.discount)}</dd></div> : null}<div className={styles.totalPaid}><dt>Amount Paid</dt><dd>{currency(payment.amountPaid)}</dd></div><div><dt>Balance</dt><dd>{currency(payment.pendingAmount)}</dd></div><div className={`${styles.paymentResult} ${resultClass}`}><dt>{status.key === "paid" ? "Paid in full" : status.label}</dt><dd>{status.key === "paid" ? <ShieldCheck size={16} /> : null}</dd></div></dl>
        </section>
