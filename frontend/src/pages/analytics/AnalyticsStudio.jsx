...
const Overview = ({ data, canManageFees }) => {
...
const AnalyticsStudio = () => {
  ...
  return <div className={`page ${styles.page}`}>
    ...
    {state.loading ? <div className={styles.loading}><RefreshCw size={28}/><strong>Building your academy insights...</strong><span>Students, attendance, finance and performance data are being analysed.</span></div> : <main className={styles.content}>{activeTab === "overview" ? <Overview data={state.data} canManageFees={state.canManageFees}/> : null}{activeTab === "students" ? <StudentsPanel data={state.data.students}/> : null}{activeTab === "attendance" ? <AttendancePanel data={state.data.attendance}/> : null}{activeTab === "fees" && state.canManageFees ? <FeesPanel data={state.data.fees}/> : null}{activeTab === "performance" ? <PerformancePanel data={state.data.performance}/> : null}</main>}
  </div>;
};
