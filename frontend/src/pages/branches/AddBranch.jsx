  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (mode === "create-another") {
        ...
        setSuccess(`${payload.branchName} created successfully. You can add another branch.`);
        window.scrollTo({ top: 0, behavior: "smooth" });
        await loadHeaderData({ quiet: true });
      } else {
        navigate("/branches", { state: { message: `${payload.branchName} created successfully.` } });
      }
    } catch (requestError) {
      setError(requestError?.response?.data?.message || "Failed to create branch. Please review the details and try again.");
      window.scrollTo({ top: 0, behavior: "smooth" });
    } finally {
      setSavingMode("");
    }
  ...
  return (
    <div className="page add-branch-page">
      <AcademyHeroHeader
        ...
        action={<button type="button" className="add-branch-hero-refresh" onClick={() => loadHeaderData({ quiet: true })} disabled={headerRefreshing}><RefreshCw size={16} className={headerRefreshing ? "is-spinning" : ""} />{headerRefreshing ? "Refreshing" : "Refresh"}</button>}
      />

      <nav className="add-branch-breadcrumb" aria-label="Breadcrumb"><Link to="/branches">Branches</Link><span>/</span><strong>Add New Branch</strong></nav>

      <header className="add-branch-heading">
        <div className="add-branch-heading__title"><span><Building2 size={25} /></span><div><h1>Add New Branch</h1><p>Create a new academy location and configure its complete operational profile.</p></div></div>
        <Link to="/branches" className="btn btn-outline"><ArrowLeft size={16} /> Back to Branches</Link>
      </header>
      ...
      <form className="add-branch-form" onSubmit={handleSubmit}>
        ...
        <footer className="add-branch-actions">
          <div><Settings2 size={17} /><span><strong>Ready to create this branch?</strong><small>Required fields are marked with an asterisk.</small></span></div>
          <button type="button" className="btn btn-outline" onClick={() => navigate("/branches")} disabled={isSaving}><X size={16} /> Cancel</button>
          <button type="submit" name="submitMode" value="create-another" className="btn btn-outline" disabled={isSaving}><Plus size={16} /> {savingMode === "create-another" ? "Saving…" : "Save & Add Another"}</button>
          <button type="submit" name="submitMode" value="create" className="btn btn-primary" disabled={isSaving}><Save size={16} /> {savingMode === "create" ? "Creating…" : "Create Branch"}</button>
        </footer>
      </form>
    </div>
  );
