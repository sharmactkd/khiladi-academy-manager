...
export default function Imports() {
  ...
  // In recovery mode an unchecked identity is an explicit "do not import"
  // choice, so it must not keep the workflow blocked as unresolved.
  const actionableUnresolved = reconciliationMode
    ? unresolved.filter((item) => recoveryStudentSelection.includes(item.key))
    : unresolved;
  ...
  const execute = async (matchedOnly = false) => {
    ...
    if (!matchedOnly && actionableUnresolved.length) { setError("Resolve pending players or choose Import verified only."); return; }
    ...
    try {
      ...
      const status = stop.current ? "paused" : totals.failed || actionableUnresolved.length ? "partial" : "completed";
