...
export default function Imports() {
  ...
  return <div className={styles.page}>
    ...
    {reconciliationMode && decisionItems.length > 0 && <section className={styles.card}>
      <div className={styles.decisionHeader}><div><h2>Missing or unresolved student records</h2><p>Only Excel students that need a new record or manual identity confirmation are shown here.</p></div><strong>{decisionItems.length} records</strong></div>
      <div className={styles.table}><table><thead><tr><th>Student</th><th>Finding</th><th>Recovery action</th></tr></thead><tbody>{decisionItems.map((item) => <tr key={item.key}><td><strong>{item.name}</strong><small>{item.phone || item.row.admissionNumber || "No identifier"}</small></td><td>{decisions[item.key] === "__new__" ? "Record is missing in the app" : suggestions[item.key]?.reason || "Identity needs confirmation"}</td><td><select disabled={Boolean(job)} value={decisions[item.key] || ""} onChange={(event) => setDecisions((values) => ({ ...values, [item.key]: event.target.value }))}><option value="">Choose recovery action</option><option value="__new__">Create missing student record</option><option value="__skip__">Exclude from recovery</option>{[...students].sort((a, b) => studentName(a).localeCompare(studentName(b), "en", { sensitivity: "base", numeric: true })).map((student) => <option key={id(student)} value={id(student)}>{studentName(student)} · {student.phone || student.admissionNumber || "No identifier"}{student.status && student.status !== "active" ? ` · ${student.status}` : ""}</option>)}</select></td></tr>)}</tbody></table></div>
    </section>}
