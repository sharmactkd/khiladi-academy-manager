...
import { directory, suggest, prepareImportChoices, importCandidates, nameVariantCandidates, chunks, attendancePayloads, id, list, norm, studentName, unwrap, safeCsv, journalSummary } from "./importLogic.js";
...
export default function Imports() {
  ...
  const [recoveryStudentSelection, setRecoveryStudentSelection] = useState([]);
  const [recoveryAttendanceSelection, setRecoveryAttendanceSelection] = useState([]);
  const [recoveryMetadataSelection, setRecoveryMetadataSelection] = useState([]);
  ...
  const openFile = async (source, restore = null) => {
    ...
    try {
      ...
      if (expected) { setResumePlan(expected); setMode(expected.mode); setBranch(expected.plan.branch); setBatch(expected.plan.batch); setScope(expected.plan.scope); setPolicy(expected.plan.policy); setDuplicateMode(expected.plan.duplicateMode); setOverrides(expected.plan.overrides || {}); setDuplicateChoices(expected.plan.duplicateChoices || {}); setRecoveryStudentSelection(expected.plan.recoveryStudentSelection || []); setRecoveryAttendanceSelection(expected.plan.recoveryAttendanceSelection || []); setRecoveryMetadataSelection(expected.plan.recoveryMetadataSelection || []); setImportTarget(expected.plan.importTarget || "all"); setReconciliationMode(Boolean(expected.plan.reconciliationMode)); }
      else { setJob(null); setResumePlan(null); setOverrides({}); setDuplicateChoices({}); setRecoveryStudentSelection([]); setRecoveryAttendanceSelection([]); setRecoveryMetadataSelection([]); setImportTarget("new"); setPolicy("keep"); setDuplicateMode("skip"); setReconciliationMode(false); }
  ...
  const pendingDuplicateGroups = duplicateGroups.filter(([nameKey]) => !duplicateChoices[nameKey]);
  ...
  // Recovery imports are selection-driven. Unresolved identities are excluded
  // by importCandidates and must not block safe, already-resolved rows.
  const blockingUnresolved = reconciliationMode ? [] : unresolved;
  const skippedRecoveryIdentities = reconciliationMode
    ? unresolved.filter((item) => recoveryStudentSelection.includes(item.key)).length
    : 0;
  const importableItems = importCandidates(selectedItems, decisions, importTarget);
  const included = useMemo(() => importableItems
    .filter((item) => !reconciliationMode || recoveryStudentSelection.includes(item.key))
    .filter((item) => !reconciliationMode || !recoveryAudit || decisions[item.key] === "__new__" || (item.attendance || []).some((row) => recoveryAttendanceSelection.includes(attendanceSourceKey(row)) || recoveryMetadataSelection.includes(attendanceSourceKey(row))))
    .map((item) => !reconciliationMode || !recoveryAudit ? item : ({ ...item, attendance: (item.attendance || []).filter((row) => recoveryAttendanceSelection.includes(attendanceSourceKey(row)) || recoveryMetadataSelection.includes(attendanceSourceKey(row))) })),
  [importableItems, reconciliationMode, recoveryStudentSelection, recoveryAudit, recoveryAttendanceSelection, recoveryMetadataSelection, decisions]);
  const variantItems = useMemo(() => selectedItems.flatMap((item) => {
    if (decisions[item.key] && !decisions[item.key].startsWith("__")) return [];
    const candidates = nameVariantCandidates(item, students, batch).filter(({ student }) => norm(studentName(student)) !== norm(item.name));
    return candidates.length ? [{ item, candidates }] : [];
  }), [selectedItems, decisions, students, batch]);
  const variantItemKeys = useMemo(() => new Set(variantItems.map(({ item }) => item.key)), [variantItems]);
  const missingRecordItems = decisionItems.filter((item) => !variantItemKeys.has(item.key) && decisions[item.key] === "__new__");
  const missingAttendanceRows = recoveryAudit?.rows?.filter((row) => row.missingCells > 0) || [];
  const metadataDifferenceRows = recoveryAudit?.rows?.flatMap((row) => (row.metadataDiffs || []).map((difference) => ({ ...difference, rowKey: row.rowKey, name: row.name, student: row.student, year: row.year, month: row.month }))) || [];
  ...
  const review = async () => {
    ...
    try {
      ...
      setRecoveryStudentSelection(selectedItems.map((item) => item.key));
      setRecoveryAttendanceSelection([]);
      setRecoveryMetadataSelection([]);
  ...
  const runRecoveryAudit = async () => {
    ...
    try {
      const combined = { rows: [], summary: { missingMonths: 0, partialMonths: 0, missingCells: 0, conflicts: 0, missingMetadataMonths: 0, metadataDifferences: 0 } };
      ...
      setRecoveryAttendanceSelection([...new Set(combined.rows.filter((row) => row.missingCells > 0).map((row) => row.rowKey))]);
      setRecoveryMetadataSelection([...new Set(combined.rows.filter((row) => row.metadataMissing).map((row) => row.rowKey))]);
    } catch (e) { setError(errorText(e)); }
    finally { setAuditBusy(false); setProgress(""); }
  };
  useEffect(() => {
    if (phase === "review" && reconciliationMode && !recoveryAudit && !auditBusy && selectedItems.length) runRecoveryAudit();
  // Run once when recovery review first opens; later identity changes use the visible Re-run comparison button.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [phase, reconciliationMode]);
  const plan = () => ({ sheetRoles, branch, batch, scope, selected, decisions, months, policy, duplicateMode, overrides, duplicateChoices, recoveryStudentSelection, recoveryAttendanceSelection, recoveryMetadataSelection, importTarget, reconciliationMode, mappings: Object.fromEntries(Object.entries(records).map(([sheet, data]) => [sheet, { headerIndex: data.headerIndex, mapping: data.mapping }])) });
  ...
  const execute = async (matchedOnly = false) => {
    ...
    if (!matchedOnly && blockingUnresolved.length) { setError("Resolve pending players or choose Import verified only."); return; }
    ...
    try {
      ...
      const status = stop.current ? "paused" : totals.failed || blockingUnresolved.length ? "partial" : "completed";
  ...
  const reset = () => { if (!busy) { setJob(null); setResumePlan(null); setFile(null); setHash(""); setRecords({}); setBlocks([]); setResult(null); setRecoveryAudit(null); setDuplicateChoices({}); setRecoveryStudentSelection([]); setRecoveryAttendanceSelection([]); setRecoveryMetadataSelection([]); setPhase("setup"); setError(""); setProgress(""); } };
  ...
  return <div className={styles.page}>
    ...
    {pendingDuplicateGroups.length > 0 && <section className={styles.card}>
      <div className={styles.decisionHeader}><div><h2>Same-name records need confirmation</h2><p>Confirm whether repeated Excel identities belong to one student or different students. A confirmed group is removed from this pending list.</p></div><strong>{pendingDuplicateGroups.length} pending</strong></div>
      <div className={styles.scopePlayerGrid}>{pendingDuplicateGroups.map(([nameKey, items]) => {
    ...
    {!reconciliationMode && <section className={`${styles.card} ${styles.matchingCard} ${styles.decisionCard}`}>
    ...
    {!reconciliationMode && matchedItems.length > 0 && <details className={`${styles.card} ${styles.matchedReview}`}><summary><span><CircleCheck size={17} /> Automatically matched players <b>{matchedItems.length}</b></span><small>Optional review</small></summary><p>These identities matched safely by admission number or exact name and phone. Use Change if a match looks wrong.</p><div className={styles.table}><table><thead><tr><th>Excel player</th><th>Matched app student</th><th>Reason</th><th>Action</th></tr></thead><tbody>{matchedItems.map(item => { const existing = students.find(student => id(student) === decisions[item.key]); return <tr key={item.key}><td><strong>{item.name}</strong><small>{item.phone || "No phone"}</small></td><td>{existing ? studentName(existing) : "Saved match"}<small>{existing?.phone || existing?.admissionNumber || ""}</small></td><td>{suggestions[item.key]?.reason || "Previously confirmed match"}</td><td><button type="button" disabled={Boolean(job)} onClick={() => { setDecisions(values => ({ ...values, [item.key]: "" })); setPage(0); }}>Change</button></td></tr>; })}</tbody></table></div></details>}
    {!reconciliationMode && excludedItems.length > 0 && <details className={`${styles.card} ${styles.excludedReview}`}><summary>Excluded players ({excludedItems.length})</summary><div className={styles.scopePlayerGrid}>{excludedItems.map(item => <div key={item.key}><span><strong>{item.name}</strong><small>{item.phone || "No identifier"}</small></span><button type="button" disabled={Boolean(job)} onClick={() => setDecisions(values => ({ ...values, [item.key]: "" }))}>Restore</button></div>)}</div></details>}
    {reconciliationMode && variantItems.length > 0 && <section className={styles.card}>
      <div className={styles.decisionHeader}><div><h2>Name and spelling variants</h2><p>Excel names that may belong to an existing app student. Confirm each merge; similarity never auto-merges records.</p></div><div className={styles.actions}><button type="button" onClick={() => setRecoveryStudentSelection((values) => [...new Set([...values, ...variantItems.map(({ item }) => item.key)])])}>Select all</button><button type="button" onClick={() => setRecoveryStudentSelection((values) => values.filter((key) => !variantItemKeys.has(key)))}>Clear all</button></div></div>
      <div className={styles.table}><table><thead><tr><th>Select</th><th>Excel identity</th><th>Possible app matches</th><th>Merge decision</th></tr></thead><tbody>{variantItems.map(({ item, candidates }) => <tr key={item.key}><td><input type="checkbox" checked={recoveryStudentSelection.includes(item.key)} onChange={() => setRecoveryStudentSelection((values) => values.includes(item.key) ? values.filter((key) => key !== item.key) : [...values, item.key])} /></td><td><strong>{item.name}</strong><small>{item.phone || item.row.admissionNumber || "No identifier"}</small><small>{item.sources.join(", ")}</small></td><td>{candidates.map(({ student, score, phoneMatch }) => <small key={id(student)}>{studentName(student)} · {phoneMatch ? "phone match" : `${Math.round(score * 100)}% name similarity`}</small>)}</td><td><select value={decisions[item.key] || ""} onChange={(event) => setDecisions((values) => ({ ...values, [item.key]: event.target.value }))}><option value="">Confirm app student</option>{candidates.map(({ student }) => <option key={id(student)} value={id(student)}>{studentName(student)} · {student.phone || student.admissionNumber || "No identifier"}</option>)}<option value="__new__">Different person — create record</option><option value="__skip__">Exclude</option></select></td></tr>)}</tbody></table></div>
    </section>}
    {reconciliationMode && missingRecordItems.length > 0 && <section className={styles.card}>
      <div className={styles.decisionHeader}><div><h2>Student records missing in app</h2><p>No safe exact or similar app identity was found for these Excel students.</p></div><div className={styles.actions}><button type="button" onClick={() => setRecoveryStudentSelection((values) => [...new Set([...values, ...missingRecordItems.map((item) => item.key)])])}>Select all</button><button type="button" onClick={() => setRecoveryStudentSelection((values) => values.filter((key) => !missingRecordItems.some((item) => item.key === key)))}>Clear all</button></div></div>
      <div className={styles.table}><table><thead><tr><th>Select</th><th>Student</th><th>Excel source</th><th>Recovery action</th></tr></thead><tbody>{missingRecordItems.map((item) => <tr key={item.key}><td><input type="checkbox" checked={recoveryStudentSelection.includes(item.key)} onChange={() => setRecoveryStudentSelection((values) => values.includes(item.key) ? values.filter((key) => key !== item.key) : [...values, item.key])} /></td><td><strong>{item.name}</strong><small>{item.phone || item.row.admissionNumber || "No identifier"}</small></td><td>{item.sources.join(", ")}</td><td><select value={decisions[item.key] || "__new__"} onChange={(event) => setDecisions((values) => ({ ...values, [item.key]: event.target.value }))}><option value="__new__">Create missing student record</option><option value="__skip__">Exclude</option>{[...students].sort((a, b) => studentName(a).localeCompare(studentName(b), "en", { sensitivity: "base", numeric: true })).map((student) => <option key={id(student)} value={id(student)}>Link: {studentName(student)} · {student.phone || student.admissionNumber || "No identifier"}</option>)}</select></td></tr>)}</tbody></table></div>
    </section>}
    {reconciliationMode && <section className={styles.card}>
      ...
      {recoveryAudit && <>
        ...
        {missingAttendanceRows.length > 0 && <><div className={styles.decisionHeader}><div><h3>Missing attendance by student and month</h3><p>Only completely or partially missing Excel attendance is listed.</p></div><div className={styles.actions}><button type="button" onClick={() => setRecoveryAttendanceSelection([...new Set(missingAttendanceRows.map((row) => row.rowKey))])}>Select all</button><button type="button" onClick={() => setRecoveryAttendanceSelection([])}>Clear all</button></div></div><div className={styles.table}><table><thead><tr><th>Select</th><th>Student</th><th>Missing month</th><th>Excel/App cells</th><th>Missing dates</th><th>Protected conflicts</th></tr></thead><tbody>{missingAttendanceRows.slice(0, 1000).map((row, index) => <tr key={`${row.rowKey}:${index}`}><td><input type="checkbox" checked={recoveryAttendanceSelection.includes(row.rowKey)} onChange={() => setRecoveryAttendanceSelection((values) => values.includes(row.rowKey) ? values.filter((key) => key !== row.rowKey) : [...values, row.rowKey])} /></td><td><strong>{row.name}</strong><small>{row.student?.name || "New/unresolved student"}</small></td><td>{row.month && row.year ? `${monthLabels[row.month]} ${row.year}` : row.sourceSheet}<small>{row.existingCells === 0 ? "Entire month missing" : "Partial month"}</small></td><td>{row.excelCells} / {row.existingCells}</td><td>{row.missingDates.slice(0, 10).join(", ")}{row.missingDates.length > 10 ? ` +${row.missingDates.length - 10}` : ""}</td><td>{row.conflicts.length}</td></tr>)}</tbody></table></div></>}
        {metadataDifferenceRows.length > 0 && <><div className={styles.decisionHeader}><div><h3>Due date, paid date and fee metadata differences</h3><p>Blank app values can be filled. Different saved app values are shown but protected from overwrite.</p></div><div className={styles.actions}><button type="button" onClick={() => setRecoveryMetadataSelection([...new Set(metadataDifferenceRows.filter((row) => row.type === "missing-in-app").map((row) => row.rowKey))])}>Select all missing</button><button type="button" onClick={() => setRecoveryMetadataSelection([])}>Clear all</button></div></div><div className={styles.table}><table><thead><tr><th>Select</th><th>Student</th><th>Month</th><th>Field</th><th>Excel value</th><th>App value</th><th>Result</th></tr></thead><tbody>{metadataDifferenceRows.slice(0, 1500).map((row, index) => <tr key={`${row.rowKey}:${row.field}:${index}`}><td><input type="checkbox" disabled={row.type !== "missing-in-app"} checked={row.type === "missing-in-app" && recoveryMetadataSelection.includes(row.rowKey)} onChange={() => setRecoveryMetadataSelection((values) => values.includes(row.rowKey) ? values.filter((key) => key !== row.rowKey) : [...values, row.rowKey])} /></td><td><strong>{row.name}</strong><small>{row.student?.name || "New/unresolved student"}</small></td><td>{row.month && row.year ? `${monthLabels[row.month]} ${row.year}` : "—"}</td><td>{row.label}</td><td>{row.excel || "—"}</td><td>{row.app || "—"}</td><td>{row.type === "missing-in-app" ? "Selected value can fill blank" : row.type === "different" ? "Different — app value protected" : "Excel blank — no change"}</td></tr>)}</tbody></table></div></>}
        {!missingAttendanceRows.length && !metadataDifferenceRows.length && <div className={styles.allResolved}><CircleCheck size={19} /><div><strong>No missing attendance or metadata found.</strong><p>Selected Excel data is already present in the app.</p></div></div>}
    ...
    {phase === "review" && <><div className={styles.actions}><button disabled={Boolean(job)} onClick={() => setPhase("mapping")}>Back to Excel columns</button><button onClick={saveDraft}>Save browser draft</button><button className={styles.primary} disabled={!included.length || (reconciliationMode && !recoveryAudit)} onClick={() => setPhase("confirm")}>Continue: review & import</button></div>{reconciliationMode && recoveryAudit && (pendingDuplicateGroups.length > 0 || skippedRecoveryIdentities > 0) && <p>Unresolved or unconfirmed identities will be safely skipped. Only the selected data linked to resolved students will continue.</p>}</>}
    {phase === "confirm" && <section className={styles.card}><h2>Review before saving</h2><div className={styles.stats}><article><small>New students to create</small><strong>{included.filter(item => decisions[item.key] === "__new__").length}</strong></article><article><small>Link to existing students</small><strong>{included.filter(item => decisions[item.key] !== "__new__").length}</strong></article><article><small>Still need a choice</small><strong>{blockingUnresolved.length}</strong></article><article><small>Excluded from import</small><strong>{selectedItems.filter(item => decisions[item.key] === "__skip__").length}</strong></article></div><p>Import: {({students:"Student records",attendance:"Attendance",both:"Records + attendance"})[mode]}. Destination: {batchOptions.find(item => id(item) === batch)?.batchName}. Attendance is imported only for identities whose student step succeeds.</p><div className={styles.actions}><button onClick={() => setPhase("review")}>Back to student matching</button><button onClick={saveDraft}>Save browser draft</button><button className={styles.primary} disabled={!included.length || Boolean(blockingUnresolved.length)} onClick={() => execute(false)}>Import selected records / attendance</button>{blockingUnresolved.length > 0 && <button disabled={!included.length} onClick={() => execute(true)}>Import ready students only — skip pending</button>}</div></section>}
