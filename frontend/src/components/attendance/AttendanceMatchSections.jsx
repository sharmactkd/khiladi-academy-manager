...
function MatchSection({ title, groups, students, options, onResolve, onCreate, busy }) {
    ...
    {!groups.length ? <p className={styles.selectionNote}>No students in this list.</p> : <>
      <div className={styles.reviewTableWrap}><table className={styles.reviewTable}>
        ...
        <tbody>{groups.slice(current * 25, (current + 1) * 25).map((group) => {
          ...
          return <tr key={group.groupKey} className={group.studentId || group.excluded ? "" : styles.unresolvedRow}>
            ...
            <td>{group.studentId ? <span className={styles.matchOk}>{student?.name || "Confirmed student"}</span> : group.excluded ? <span className={styles.excluded}>Excluded</span> : <span className={styles.matchError}>Review · {group.reason}</span>}</td>
            <td><select aria-label={`Confirm student for ${group.name}`} disabled={busy}
              value={group.excluded ? "__skip__" : group.studentId || ""}
...
export default function AttendanceMatchSections({ groups, availableStudents, onResolve, onCreate, busy }) {
  ...
  const saveRecord = async () => {
    ...
    try {
      ...
      setCreatingGroup(null); setActiveTab("matched");
    } catch (failure) {
  ...
  return <>
    <div className={styles.matchTabs} role="group" aria-label="Student matching lists">
      ...
      <button type="button" aria-pressed={activeTab === "unmatched"} onClick={() => setActiveTab("unmatched")}>Unmatched ({groups.filter((group) => !group.studentId).length})</button>
    </div>
    ...
    <MatchSection key={`${activeTab}:${search}`} title={activeTab === "matched" ? "Matched students" : "Unmatched / Needs review"} groups={filtered.filter((group) => activeTab === "matched" ? group.studentId : !group.studentId)} students={students} options={options} onResolve={onResolve} onCreate={openCreate} busy={busy} />
  </>;
}
