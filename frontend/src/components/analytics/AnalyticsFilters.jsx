...
const AnalyticsFilters = ({ branches, filters, loading, onChange, onRefresh }) => {
  ...
  return (
    <section className="analytics-studio__filters" aria-label="Analytics filters">
      ...
      <label className="analytics-studio__field"><span>From date</span><DateInput value={filters.fromDate} max={filters.toDate || undefined} onChange={(event) => patch({ fromDate: event.target.value, preset: "custom" })} /></label>
      <label className="analytics-studio__field"><span>To date</span><DateInput value={filters.toDate} min={filters.fromDate || undefined} onChange={(event) => patch({ toDate: event.target.value, preset: "custom" })} /></label>
