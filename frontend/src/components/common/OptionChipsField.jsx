...
const OptionChipsField = ({ allowCustom = false, className = "", customInput = "", customPlaceholder = "Add custom option", customValues = [], label, multiple = true, onAddCustom, onCustomInputChange, onChange, options = [], value = [] }) => {
  ...
  return <div className={clsx(styles.root, className)}>{label ? <span className={styles.label}>{label}</span> : null}<div className={clsx(styles.chips, allOptions.length === 1 && styles.single)}>{allOptions.map((option) => { const active = multiple ? selected.includes(option) : selected === option; return <button type="button" key={option} className={clsx(styles.chip, active && styles.selected)} aria-pressed={active} onClick={() => toggle(option)}>{active ? <Check size={13} /> : <Plus size={13} />}{option}</button>; })}</div>{allowCustom ? <div className={styles.custom}><input value={customInput} placeholder={customPlaceholder} onChange={(event) => onCustomInputChange?.(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); onAddCustom?.(); } }} /><button type="button" disabled={!customInput.trim()} onClick={onAddCustom}><Plus size={14} /> Add</button></div> : null}</div>;
};
