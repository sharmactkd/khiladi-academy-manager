...
const ReportFilters = ({ batches, branches, filters, onChange, onGenerate, loading, reportType, styles }) => {
  ...
  return <section className={styles.filterCard}>
    ...
    <div className={styles.filterGrid}>
      ...
      {!reportType.noDates ? <><label><span>From date</span><input type="date" value={filters.fromDate} max={filters.toDate || undefined} onChange={(event) => patch({ fromDate: event.target.value, preset: "custom" })}/></label><label><span>To date</span><input type="date" value={filters.toDate} min={filters.fromDate || undefined} onChange={(event) => patch({ toDate: event.target.value, preset: "custom" })}/></label></> : null}
