...
const formatValue = (value, key = "", currencySource = {}) => {
  ...
  if (/amount|paid|pending/i.test(key) && !Number.isNaN(Number(value))) return formatMoney(value, currencySource);
...
const ReportDocument = ({ report, search = "", onSearch, visibleKeys, onVisibleKeys, styles, compact = false }) => {
  ...
  return <section id="report-preview" className={styles.document}>
    ...
    <div className={styles.tableWrap}><table><thead><tr><th>No.</th>{columns.map((column) => <th key={column.key}>{column.label}</th>)}</tr></thead><tbody>{rows.length ? rows.map((row, index) => <tr key={`${index}-${row._id || "row"}`} className={compact && index >= previewLimit ? documentStyles.compactHidden : ""}><td>{index + 1}</td>{columns.map((column) => <td key={column.key} className={/status|result|medal/i.test(column.key) ? styles.statusCell : ""}>{formatValue(row[column.key], column.key, row)}</td>)}</tr>) : <tr><td colSpan={columns.length + 1}><div className={styles.emptyRows}><FileSearch size={23}/><strong>No matching records</strong><span>Change the filters or search term.</span></div></td></tr>}</tbody></table></div>
