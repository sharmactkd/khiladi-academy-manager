...
export const listExpenses = asyncHandler(async (req, res) => {
  ...
  const transactions = await ExpenseTransaction.find(filter)
...
export const createExpense = asyncHandler(async (req, res) => {
  ...
  if (
    !["income", "expense"].includes(payload.type) ||
    !payload.category ||
    ...
    return res.status(400).json({
      success: false,
      message: "Type, category and a positive amount are required.",
    });
  }
  ...
  const transaction = await ExpenseTransaction.findOneAndUpdate(
    ...
    },
    {
      ...
      reversalReason: String(
        req.body.reason || "Reversed by manager",
      ),
    },
    { new: true },
  );
