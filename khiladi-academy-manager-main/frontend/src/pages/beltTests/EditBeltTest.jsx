...
import { ArrowLeft, ArrowRight, Award, BadgeCheck, CalendarDays, ClipboardCheck, FileBadge2, GraduationCap, History, Image, Medal, RotateCcw, ShieldCheck, Sparkles, Trophy, UserRound, X } from "lucide-react";
  ...
  return <div className={`page ${baseStyles.page}`}>
    ...
    {record ? <form className={baseStyles.workspace} onSubmit={handleSubmit}>
      <main className={baseStyles.formCard}>
        ...
        <section className={baseStyles.section}><header><span>03</span><div><h2>Assessment Result</h2><p>Update test date, marks and final grading outcome.</p></div></header><div className={styles.assessmentPanel}><div className={baseStyles.assessmentGrid}><label><span>Test Date *</span><div className={`${baseStyles.iconInput} ${styles.iconControl}`}><CalendarDays size={15}/><DateInput name="testDate" value={form.testDate} onChange={handleChange} required/></div></label><label><span>Marks Obtained</span><input type="number" min="0" step="0.01" name="marks" value={form.marks} onChange={handleChange} placeholder="Obtained marks"/></label><label><span>Total Marks</span><input type="number" min="0.01" step="0.01" name="outOf" value={form.outOf} onChange={handleChange} placeholder="Out of"/></label><div className={`${baseStyles.scoreBox} ${styles.scoreBox}`}><small>Live Score</small><strong>{score === null ? "—" : `${score}%`}</strong><span>{score === null ? "Add marks" : "Calculated"}</span></div></div><fieldset className={`${baseStyles.resultField} ${styles.resultField}`}><legend>Result *</legend><div>{[{ value: "pending", label: "Pending", icon: ClipboardCheck }, { value: "pass", label: "Pass", icon: BadgeCheck }, { value: "fail", label: "Fail", icon: X }].map(({ value, label, icon: Icon }) => <button key={value} type="button" className={form.result === value ? baseStyles[`result${label}`] : ""} onClick={() => update("result", value)}><span className={styles.resultIcon}><Icon size={16}/></span><span>{label}</span></button>)}</div></fieldset></div></section>
