...
import PersonContactRepeater, {
  createPersonContact,
} from "../../components/common/PersonContactRepeater.jsx";
...
const normalizeOptions = (value) => {
  if (Array.isArray(value))
    return [...new Set(value.flatMap(normalizeOptions))];
...
const EditStudent = () => {
  ...
  useEffect(() => {
    if (beltRank !== "Black") setValue("danRank", "");
  }, [beltRank, setValue]);
  useEffect(() => {
    let mounted = true;
    setLoading(true);
    Promise.allSettled([
      ...
      .then(([studentResult, academyResult, branchResult, batchResult]) => {
        if (studentResult.status !== "fulfilled") {
          toast.error(
            studentResult.reason?.response?.data?.message ||
        ...
        const studentData =
          studentResult.value?.data?.data?.student ||
        ...
        reset({
          branch: normalizeEntityId(studentData?.branch),
          batch: normalizeEntityId(studentData?.batch),
          ...
          collegeName:
            ...
            "",
          occupation:
            studentData?.occupation || studentData?.education?.occupation || "",
          heightCm:
            studentData?.heightCm ?? studentData?.physicalInfo?.heightCm ?? "",
          weightKg:
            studentData?.weightKg ?? studentData?.physicalInfo?.weightKg ?? "",
          medicalNotes:
        ...
        setParentContacts(
          normalizePersonContacts(studentData?.parentContacts, {
        ...
        setEmergencyContacts(
          normalizePersonContacts(studentData?.emergencyContacts, {
            name:
              studentData?.emergencyContact?.name ||
              studentData?.emergencyContactName,
            countryCode:
              studentData?.emergencyContact?.countryCode ||
              studentData?.emergencyContactCountryCode,
            phone:
              studentData?.emergencyContact?.phone ||
              studentData?.emergencyContactPhone,
          }),
        );
        const medicalSelection = normalizeMedicalSelection(
          normalizeOptions(
        ...
        if (academyResult.status === "fulfilled") {
          ...
          if (sports.length && !studentData?.martialArt)
            setValue("martialArt", sports[0]);
        }
        if (branchResult.status === "fulfilled") {
          const activeBranches = unwrapList(
            branchResult.value,
            "branches",
          ).filter((branch) => branch?.isActive !== false);
          setBranches(activeBranches);
          if (activeBranches.length === 1 && !studentData?.branch) {
            setValue("branch", normalizeEntityId(activeBranches[0]), {
              shouldValidate: true,
            });
          }
        }
        if (batchResult.status === "fulfilled") {
          const activeBatches = unwrapList(batchResult.value, "batches").filter(
            (batch) => batch?.isActive !== false,
          );
          setBatches(activeBatches);
          if (activeBatches.length === 1 && !studentData?.batch) {
            setValue("batch", normalizeEntityId(activeBatches[0]), {
              shouldValidate: true,
            });
          }
        }
  ...
  const contactUpdater = (setter) => (field, value) =>
  ...
  const mainBranch =
    branches.find((branch) => branch?.isMainBranch) || branches[0];
  ...
  const onSubmit = async (values) => {
    try {
      setSaving(true);
      setSubmitError("");
      const names = String(values.name || "")
        .trim()
        .split(/\s+/);
      const payload = {
        ...
        parentContacts: parentContacts.map(
          ({ id, customRelation, ...contact }) => ({
            ...contact,
            relation:
              contact.relation === "Other" ? customRelation : contact.relation,
          }),
        ),
        ...
        medicalConditions: buildMedicalConditionsPayload(
          medicalConditions,
  ...
  if (loading)
    return (
      <div className="page student-form-page">
        <div className="student-form-loading" role="status">
  ...
  return (
    <div className="page add-branch-page student-form-page">
      <AcademyHeroHeader
      ...
      <div className="add-branch-heading">
        <div className="add-branch-heading__title">
          ...
          <div>
            <h1>Edit Student</h1>
            <p>
              Update the complete student profile with the same professional
              workflow.
            </p>
          </div>
        </div>
      ...
      >
              ...
              <div className="student-assignment-field">
                ...
                {branches.length > 1 ? (
                  <select {...register("branch")}>
                    ...
                    {branches.map((branch) => {
                      const branchId = normalizeEntityId(branch);
                      return (
                  ...
                  <strong>
                    {branches[0]?.branchName || "No Active Branch"}
                  </strong>
                )}
              </div>
              <div className="student-assignment-field">
                ...
                {batches.length > 1 ? (
                  <select {...register("batch")}>
                    ...
                    {batches.map((batch) => {
                      const batchId = normalizeEntityId(batch);
                      return (
                  ...
                  <strong>{batches[0]?.batchName || "No Active Batch"}</strong>
                )}
              </div>
              <div className="student-status-field">
                ...
                <label className="add-branch-switch student-status-switch">
                  ...
                  <div>
                    ...
                    <small>
                      {status === "active"
                        ? "Keep this student available for academy operations."
                        : "Student remains unavailable for academy operations."}
                    </small>
                  </div>
                </label>
              </div>
        ...
        >
          <div className="student-form-identity">
            <div className="student-form-fields student-form-fields--three">
              <label>
                <span>
                  Full Name <b>*</b>
                </span>
              ...
              <label>
                ...
                <input
                  placeholder="Existing number remains if left blank"
                  {...register("admissionNumber")}
                />
                {errorFor("admissionNumber")}
              </label>
              <label>
              ...
              <div className="student-chip-field student-gender-field">
                <OptionChipsField
                  label="Gender"
                  multiple={false}
                  options={["Male", "Female"]}
                  value={gender === "female" ? "Female" : "Male"}
                  onChange={(value) => setValue("gender", value.toLowerCase())}
                />
              </div>
              <div className="student-demographics-row">
                <label>
                  <span>Joining Date</span>
                  <DateInput {...register("joiningDate")} />
                </label>
                <label>
            ...
            <ProfilePhotoField
              ...
              onRemove={() => {
                setPhoto(null);
                setPhotoPreview(
                  student?.profilePhoto ? getStudentPhotoUrl(student) : "",
                );
              }}
              disabled={saving}
            />
        ...
        <StudentFormSection
          className="student-contact-card"
          eyebrow="CONTACT"
          title="Contact & Location"
          description="Primary contact and residential location."
          icon={MapPin}
        >
          <div className="student-form-body">
            <PhoneLocationFields
              ...
              phoneTrailingContent={
                <label className="student-contact-email">
                  <span>Email</span>
                  <input
                    type="email"
                    autoComplete="email"
        ...
        <div className="student-form-grid student-people-contact-grid">
          <StudentFormSection
            ...
            icon={Phone}
          >
            <PersonContactRepeater
              items={parentContacts}
              onChange={setParentContacts}
              addLabel="Add More Parent / Guardian"
            />
          </StudentFormSection>
          <StudentFormSection
            eyebrow="SAFETY"
            title="Emergency Contact"
            description="Contacts to use in an urgent situation."
            icon={ShieldAlert}
          >
            <PersonContactRepeater
              items={emergencyContacts}
              onChange={setEmergencyContacts}
              addLabel="Add More Emergency Contact"
            />
          </StudentFormSection>
        </div>
        <div className="student-form-grid student-training-medical-grid">
          <StudentFormSection
            ...
            icon={BookOpen}
          >
            <div className="batch-form-page student-training-components">
             <div
...
>
                <SportsMartialArtsField
                  ...
                  showHeader={false}
                  allowCustom={false}
                  options={academyMartialArts}
                  selected={martialArt ? [martialArt] : []}
                  customOptions={[]}
                  onChange={(items) =>
                    setValue("martialArt", items.at(-1) || "")
                  }
                />
                <div className="batch-limit-field--belt student-belt-field">
                  <BeltTagsField
                    ...
                    onChange={(value) =>
                      setValue("beltRank", value === beltRank ? "" : value, {
                        shouldDirty: true,
                      })
                    }
                  />
                ...
                {showBeltSelect && beltRank === "Black" ? (
                  <label className="student-dan-rank">
                    <span>Dan Rank</span>
          ...
          <StudentFormSection
            ...
            description="Health details important for safe training."
            icon={HeartPulse}
          >
        ...
        {submitError ? (
          <div className="student-submit-error" role="alert">
            <ShieldAlert size={18} />
            <span>
              <strong>Student could not be updated.</strong>
              {submitError}
            </span>
          </div>
        ) : null}
