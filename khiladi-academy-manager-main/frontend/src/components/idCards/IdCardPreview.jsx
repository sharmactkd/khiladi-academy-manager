...
import { getTemplateSize, normalizeTemplate } from "../../pages/idCards/idCardTemplate.config.js";
...
const IdCardPreview = ({ idCard = {}, template: templateProp, academy: academyProp, side = "front", compact = false }) => {
  ...
  const backgroundImage = getFileUrl(design?.backgroundImage, "");
  ...
  const cssVars = useMemo(() => ({ "--id-primary": template.primaryColor, "--id-secondary": template.secondaryColor, "--id-accent": template.accentColor, "--id-bg": template.backgroundColor, "--id-text": template.textColor, "--id-font": template.fontFamily, "--id-card-ratio": `${size.widthMm}/${size.heightMm}`, "--id-card-width-mm": `${size.widthMm}mm`, "--id-card-height-mm": `${size.heightMm}mm`, "--id-card-background": backgroundImage ? `url("${backgroundImage}")` : "none" }), [backgroundImage, size.heightMm, size.widthMm, template]);
  ...
  return <article className={cls} style={cssVars} aria-label={`${side} ID card preview`}>
    <div className={styles.background}/><div className={styles.band}/><header>{logo ? <img src={logo} alt="Academy logo"/> : <ShieldCheck/>}<div><span>{design?.label || "STUDENT IDENTITY"}</span><strong>{academy.academyName || academy.name || "KHILADI ACADEMY"}</strong></div><BadgeCheck className={styles.verified}/></header>
