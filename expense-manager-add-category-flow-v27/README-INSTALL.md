...
```powershell
Expand-Archive `
  -Path "$env:USERPROFILE\Downloads\expense-manager-add-category-flow-v27.zip" `
...
```powershell
Copy-Item `
  ".\expense-manager-add-category-flow-v27\frontend\src\*" `
  ".\frontend\src\" `
  -Recurse -Force

Copy-Item `
  ".\expense-manager-add-category-flow-v27\backend\src\*" `
  ".\backend\src\" `
  -Recurse -Force
```
